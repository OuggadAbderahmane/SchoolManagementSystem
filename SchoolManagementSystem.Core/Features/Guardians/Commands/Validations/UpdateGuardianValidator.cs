...
{
    public class UpdateGuardianValidator : AbstractValidator<UpdateGuardianCommand>
    {
        #region Fields
        ...
        #region Constructors
        public UpdateGuardianValidator(IGuardianService guardianService, IPersonService personService, IJobService jobService, IStringLocalizer<SharedResource> stringLocalizer)
        {
        ...
        public void ApplyCustuomValidationsRules()
        {
            ...
            RuleFor(x => x.JobId)
                    ...
                    {
                        if (Key != null)
                            return await _jobService.IsIdExistAsync((int)Key);
                        return true;
                    })
                    .WithMessage("{PropertyName} " + _stringLocalizer[SharedResourcesKey.DoesNotExist]);
